...
int main(int argc, char *argv[])
{
    ...
    /*(10) distvec의 출력파일은 output_dv.txt로 한다.*/
...
{
    /*(12) Tie-breaking rule 1 (distance-vector): distance vector에서는 이웃 노드로부터 라우팅 테이블을
받아서, 어떤 목적지에 대하여 지금 현재 내가 가지고 있는 경로보다 더 cost가 적은 경로가 있는
지를 확인하게 된다. cost가 같은 경우에는 ID 값이 작은 노드를 다음 노드로 선택한다. */
    for (int iter = 0; iter < node_num - 1; iter++)
    {
        for (int u = 0; u < node_num; u++)
        {
            for (map<int, int>::iterator it = edge[u].begin(); it != edge[u].end(); ++it)
            {
                ...
                {
                    if (route_table[v].cost[w] != INT_MAX)
                    {
                        if ((route_table[u].cost[w] > cost_uv + route_table[v].cost[w]) ||
                            (route_table[u].cost[w] == cost_uv + route_table[v].cost[w] && route_table[u].next[w] > v))
                        {
                            route_table[u].cost[w] = cost_uv + route_table[v].cost[w];
                            route_table[u].next[w] = v;
                            route_table[u].path[w].clear();
                            route_table[u].path[w] = route_table[v].path[w];
                            route_table[u].path[w].push_front(u);
                        }
                    }
                }
            }
...
/*print routing table <- 나중에는 텍스트 파일에 저장해야함*/
